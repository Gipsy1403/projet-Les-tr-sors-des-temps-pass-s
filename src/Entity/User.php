...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
     ...
     */
    #[ORM\OneToMany(targetEntity: Umbrella::class, mappedBy: 'user')]
    private Collection $umbrellas;
    ...
    public function addUmbrella(Umbrella $umbrella): static
    {
        if (!$this->umbrellas->contains($umbrella)) {
            $this->umbrellas->add($umbrella);
            $umbrella->setUser($this);
        }

        return $this;
    }

    public function removeUmbrella(Umbrella $umbrella): static
    {
        if ($this->umbrellas->removeElement($umbrella)) {
            // set the owning side to null (unless already changed)
            if ($umbrella->getUser() === $this) {
                $umbrella->setUser(null);
            }
        }
